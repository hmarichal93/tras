...
from tras.app import MainWindow
from tras.config import get_config
from tras.utils import newIcon
...
def _setup_loguru(logger_level: str) -> None:
    ...
    cache_dir: str
    if os.name == "nt":
        cache_dir = os.path.join(os.environ["LOCALAPPDATA"], "tras")
    else:
        cache_dir = os.path.expanduser("~/.cache/tras")

    os.makedirs(cache_dir, exist_ok=True)

    log_file = os.path.join(cache_dir, "tras.log")
    logger.add(
        log_file,
        colorize=True,
        level="DEBUG",
...
def _handle_exception(exc_type, exc_value, exc_traceback):
    ...
    QtWidgets.QMessageBox.critical(
        None,
        "Error",
        f"An unexpected error occurred. The application will close.<br/><br/>Please report issues following the <a href='https://tras.io/docs/troubleshoot'>Troubleshoot</a>.<br/><br/>{traceback_html}",  # noqa: E501
    )
...
def main():
    ...
    default_config_file = os.path.join(os.path.expanduser("~"), ".trasrc")
    parser.add_argument(
        "--config",
        dest="config",
